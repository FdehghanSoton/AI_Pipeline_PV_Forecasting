"""Ensemble-weight composition figure.

Reruns both backtests and captures the renormalised non-negative least-squares
(NNLS) stacking weights fitted on each fold's validation rows, by wrapping
``build_ensembles``. The figure shows, per protocol, the mean (+/- sd across
folds) weight the default ensemble assigns to each base learner -- an
interpretability view of which learners the stack actually relies on.
"""
...
def _capturing_build_ensembles(val_preds, y_val, val_mask, test_preds):
    ...
    _captured[_current_mode["mode"]].append(dict(zip(names, w)))
...
def main() -> None:
    ...
    for ax, mode in zip(axes, ("KFOLD", "TEMPORAL")):
        ...
        ax.set_xticks(xpos)
        ax.set_xticklabels([LABELS[b] for b in base], fontsize=7.5)
        ax.set_title(f"{MODE_LABEL[mode]}  ($n={len(folds)}$ folds)")
        ax.grid(axis="y", alpha=0.25)
        for x, v in zip(xpos, mean):
    ...
    for mode in ("KFOLD", "TEMPORAL"):
        ...
        for b, w in zip(base, mat.mean(axis=0)):
            print(f"  {b:14s} {w:.3f}")
