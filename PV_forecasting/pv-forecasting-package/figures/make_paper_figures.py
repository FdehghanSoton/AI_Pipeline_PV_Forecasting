"""Create publication-ready figures from V4 output files.

Run ``python analyze_pv_v4.py`` first. This script deliberately reads the
saved experiment outputs rather than embedding metric or correlation values in
source code, so figures cannot silently become stale when the model is rerun.
"""
...
MODEL_INFO = {
    "Ridge": ("Ridge", False),
    "GBM": ("GBM", False),
    "GBM_kt": ("Clearness GBM", False),
    "GBM_per_hour": ("Per-hour GBM", False),
    "CNN": ("CNN", False),
    "Mean": ("Mean", True),
    "InvRMSE": ("Inverse RMSE", True),
    "RidgeStack": ("Ridge stack", True),
    "NNLSStack": ("NNLS stack", True),
}
...
def figure_residual_corr() -> None:
    ...
    for panel_index, (ax, (title, matrix)) in enumerate(
        ...
        ax.set_yticklabels(labels if panel_index == 0 else [], fontsize=7)
...
def figure_representative_week() -> None:
    ...
    configurations = [
        ("KFOLD", "NNLSStack", "Random day-fold: NNLS stack"),
        ("TEMPORAL", "RidgeStack", "Rolling-origin: ridge stack"),
    ]

    fig, axes = plt.subplots(2, 1, figsize=(7.2, 4.2), sharex=False)
    for ax, (mode, model, title) in zip(axes, configurations, strict=False):
