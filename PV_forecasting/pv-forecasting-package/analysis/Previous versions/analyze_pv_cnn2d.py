...
def build_supervised_tensors(
    ...
    -------
    X
        Inputs with channels: normalised PV, past/future indicator, PV-data
        availability indicator, and weather channels.
    Y
    ...
    for d in range(history_days, n_days):
        ...
        loss_weight = valid.astype(np.float32) * (
            1.0 + 3.0 * daylight.astype(np.float32)
        )
...
def plot_scatter(preds_df: pd.DataFrame, capacity: float, out_png: Path) -> None:
    ...
    ax.set_title(
        ...
        f"nRMSE={float(np.sqrt(mean_squared_error(preds_df['y'], preds_df['yhat']))) / capacity * 100:.1f}% cap"
    )
