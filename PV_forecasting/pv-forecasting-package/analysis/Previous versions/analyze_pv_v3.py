...
def load_weather_full() -> pd.DataFrame:
    meta_path = WEATHER_CACHE.with_suffix(".meta.json")
    if not meta_path.exists():
        warnings.warn(
            "weather_cache.csv has no provenance metadata. Do not describe it "
            "as ERA5 unless the cache is regenerated with PV_WEATHER_MODEL=era5.",
            RuntimeWarning,
            stacklevel=2,
        )
    ...
    have = [c for c in EXTENDED_WEATHER if c in w.columns]
    return w[have]
