...
def fit_cnn_for_fold(
    ...
    The two mask arguments are forwarded to
    :func:`analyze_pv_cnn2d.build_supervised_tensors`; they exist so the
    missingness-handling ablation can switch each use of the mask off
    independently.
    """
    ...
    X, Y, W, _E, target_days = build_supervised_tensors(
        ...
        include_availability_channel=include_availability_channel,
        weight_missing_targets=weight_missing_targets,
    )
...
def fit_one_fold(
    ...
    grid cell is overcast and the site is not. Pass ``float("inf")`` to
    disable the bound.
    """
    ...
    kt = np.clip(y_tr / denom_tr, 0, kappa_clip)
    okkt = poa_tr > 50
    if okkt.sum() > 200:
        ...
        kt_val_p = np.clip(gbmkt.predict(X_val), 0, kappa_clip)
        kt_te_p = np.clip(gbmkt.predict(X_te), 0, kappa_clip)
    ...
    for offset in cnn_seeds:
        cnn_val_map, cnn_te_map = fit_cnn_for_fold(
            ...
            include_availability_channel=cnn_availability_channel,
            weight_missing_targets=cnn_weight_missing_targets,
        )
        cnn_val_runs.append(
...
def temporal_backtest(
    ...
    for fold in range(n_folds):
        ...
        preds = fit_one_fold(
            ...
            kappa_clip=cfg.kappa_clip,
            cnn_availability_channel=cfg.cnn_availability_channel,
            cnn_weight_missing_targets=cfg.cnn_weight_missing_targets,
        )
        out.append(
            FoldRes(
                name=f"T{fold + 1}",
...
def build_dataset(cfg: RunConfig) -> tuple[pd.DataFrame, list[str], float]:
    ...
    wx = load_weather_full(
        source=cfg.weather_source, variable_set=cfg.weather_variable_set
    )
