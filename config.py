...
class RunConfig:
    ...
    cnn_availability_channel
        Whether the CNN receives the binary channel marking which historical PV
        values were actually observed. With it off, a filled zero is
        indistinguishable from a genuine zero.
    cnn_weight_missing_targets
        Whether hours with no PV observation contribute to the CNN's training
        loss. The default excludes them; setting this to ``True`` reproduces the
        naive fill-with-zero approach. ``run_mask_ablation.py`` varies both.
    """
    ...
    weather_source: str = "ifs"
    weather_variable_set: str = "full"
    kappa_clip: float = 1.5
    cnn_availability_channel: bool = True
    cnn_weight_missing_targets: bool = False
    kfold_n_folds: int = 5
    ...
    def describe(self) -> dict[str, object]:
        ...
        return {
            ...
            "weather_variable_set": self.weather_variable_set,
            "kappa_clip": self.kappa_clip,
            "cnn_availability_channel": self.cnn_availability_channel,
            "cnn_weight_missing_targets": self.cnn_weight_missing_targets,
...
def load_config() -> RunConfig:
    ...
    return RunConfig(
        ...
        weather_source=_env_str("PV_WEATHER_SOURCE", defaults.weather_source),
        weather_variable_set=_env_str(
            "PV_WEATHER_VARIABLE_SET", defaults.weather_variable_set
        ),
        kappa_clip=_env_float("PV_KAPPA_CLIP", defaults.kappa_clip),
        cnn_availability_channel=_env_flag(
            "PV_CNN_AVAILABILITY_CHANNEL", defaults.cnn_availability_channel
        ),
        cnn_weight_missing_targets=_env_flag(
            "PV_CNN_WEIGHT_MISSING_TARGETS", defaults.cnn_weight_missing_targets
        ),
